...
// Base direcotory of the logs folder
...
lib.append = (file, str, callback) => {
    ...
    fs.open(`${lib.baseDir}${file}.log`, 'a', (err, fileDescriptor) => {
        if (!err && fileDescriptor) {
            // Append to the file for appending
...
lib.compress = (logId, newFileId, callback) => {
    ...
    fs.readFile(lib.baseDir + sourceFile, 'utf8', (err, fileContent) => {
        if (!err && fileContent) {
            // Compress the fileContent
                        ...
                        if (!err && fileDescriptor) {
                            ...
                            fs.writeFile(fileDescriptor, buffer.toString('base64'), (err) => {
                                if (!err) {
                                    // Close the destFile
                                    fs.close(fileDescriptor, (err) => {
                                        return !err ? callback(false) : callback(err)
                                    })
                                } else {
                                    callback(err)
                                }
